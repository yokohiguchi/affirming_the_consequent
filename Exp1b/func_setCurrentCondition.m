...
switch phase
    ...
    case {2} % familiarity test
        ...
        switch c.cond
            ...
            case {2}
                c.setItem = fliplr(p.set(c.setID).setItem); %CBA
                c.setCat = fliplr(p.set(c.setID).cat);
                c.testItem = [p.set(c.foilID2).setItem(3) p.set(c.foilID1).setItem(2) p.set(c.setID).setItem(1)]; %GEA
                c.testCat = [p.set(c.foilID2).cat(3) p.set(c.foilID1).cat(2) p.set(c.setID).cat(1)];
        end
