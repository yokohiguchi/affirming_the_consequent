...
if sess == 1
    d.session(sess)=sess;
    d.t.exposeStartTime = datestr(now,30);
    ...
    while 1
        ...
        if fname==0
            ...
            if strcmpi(yn,'y')
                d.session(sess)=sess;
                eval(['d.t.exposeStartTime' num2str(d.session(end)) ' = datestr(now,30)']);
                break;
            else
                clear p d;
            end
...
%     ListenChar(2);
    ...
    %% closigng experiment
    % ===================================================================
    eval(['d.t.exposeFinishTime' num2str(d.session(end)) ' = datestr(now,30)']);
    
    
    %	save the results & parameters
    % ++++++++++++++++++++++++++++++++++++++
    save(strcat('result/',d.dataFileName,'.mat'),'p','d');
    save(strcat('backup/learn/',d.dataFileName,'_learn.mat'),'p','d');
    ...
    d.t.exposeAbortTime=datestr(now,30);
